...
export default function SupplierPaymentsPage() {
  ...
  useEffect(() => {
    apiFetch<{ batches: SettlementBatch[] }>("/liff/supplier/settlements")
  ...
  const totalPaid = batches
    ...
    .reduce((s, b) => s + Number(b.netAmount), 0);

  const totalPending = batches
    .filter((b) => ["PENDING_APPROVAL", "APPROVED", "PAYMENT_ORDERED"].includes(b.status))
    .reduce((s, b) => s + Number(b.netAmount), 0);
  ...
  return (
    <div className="space-y-4 p-4">
      <h1 className="pt-1 text-lg font-bold">รายได้และการชำระเงิน</h1>

      {/* Summary cards */}
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl bg-emerald-50 border border-emerald-100 p-4">
          <p className="text-xs text-emerald-700 font-medium">ชำระแล้วทั้งหมด</p>
